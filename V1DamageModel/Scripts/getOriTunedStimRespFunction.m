...
% deterministic
    ...
    oriSpace = linspace(-pi, pi, 361); %linspace(-pi, pi, 501);
    
%     assert(sum(rad2deg(oriSpace) == rad2deg(stimParams.stimOri)) == 1)
%     
%     assert(all(ismember(neuronsPrefOrientations, oriSpace)))
%     
%     assert(numel(unique(params.d)) == 1)
%     assert(numel(unique(params.alpha)) == 1)
%     assert(numel(unique(params.b)) == 1)
%     assert(numel(unique(params.w)) == 1)
% 
%     % Step 1: Get unnormalized tuning response of neurons based on their preferred orientation.
%     % Generate tuning fn for just one orientation and for other just
%     % shift it. This is done to avoid numerical issues(
%     % sum(unnormalizedResp, 2) seems to depend upon orientation. Ideally it
%     % should not but it does because the function is shift invariant.
%     paramsCopy       = params;
%     paramsCopy.d     = params.d(1);
%     paramsCopy.alpha = params.alpha(1);
%     paramsCopy.b     = params.b(1);
%     paramsCopy.w     = params.w(1);
%     
%     baseResp = getUnormalizedTunedResponse( ...
%         oriSpace, neuronsPrefOrientations(1), params); % 1 x nOris
%     dtheta = oriSpace(2) - oriSpace(1);
%     prefIdx = round((neuronsPrefOrientations - oriSpace(1)) / dtheta) + 1;
%     nOri = numel(oriSpace);
%     baseIdx = 1:nOri;
%     shiftedIdx = mod(baseIdx - prefIdx(:) + nOri - 1, nOri) + 1;
%     unnormalizedResp = baseResp(shiftedIdx);
    
    unnormalizedResp = getUnormalizedTunedResponse( ...
       oriSpace, neuronsPrefOrientations, params); % nNeurons x nOris
    
    % Step2: Stim response f(S)
    ...
    % stimfltResp = interp1(oriSpace, unnormalizedResp', stimParams.stimOri)'; %unnormalizedResp(:, oriSpace == stimParams.stimOri);
...
function [FR] = getUnormalizedTunedResponse(orientations, neuronsPrefOrientations, params)
    ...
    % This should be circular distance instead
    ...
    % Non-linearity introduces variances across different preferred orientations of neurons.
